...
class SuperPointFrontend(object):
    ...
    def nms_fast(self, in_corners, H, W, dist_thresh):
        ...
        # Pad the border of the grid, so that we can NMS points near the
        # border.
        pad = dist_thresh
        grid = np.pad(grid, ((pad, pad), (pad, pad)), mode='constant')
        # Iterate through points, highest to lowest conf, suppress
        # neighborhood.
        count = 0
        for i, rc in enumerate(rcorners.T):
...
class VideoStreamer(object):
    ...
    def __init__(self, basedir, camid, height, width, skip, img_glob):
        ...
        if basedir == "camera/" or basedir == "camera":
            ...
            if (isinstance(self.cap, list) or not self.cap.isOpened()) and (
                    lastbit == '.mp4'):
                raise IOError('Cannot open movie file')
            elif not isinstance(self.cap, list) and self.cap.isOpened() and (lastbit != '.txt'):
    ...
    def lives():
        ...
        def gen():
            ...
            while True:
                ...
                # Get tracks for points which were match successfully across
                # all frames.
                tracks = tracker.get_tracks(opt.min_length)

                # Primary output - Show point tracks overlayed on top of input
                # image.
                ...
                # Resize final output.
                if opt.show_extra:
                    outfinal = np.hstack((out1, out2, out3))
                    outfinal = cv2.resize(
